...
export function VocabularyDeck({ level }: VocabularyDeckProps) {
  ...
  if (loading) {
    return (
      <div className="min-h-[320px] bg-white rounded-2xl border border-slate-100 shadow-sm flex flex-col items-center justify-center gap-3 text-slate-400">
  ...
  if (error || cards.length === 0) {
    return (
      <div className="min-h-[320px] bg-white rounded-2xl border border-slate-100 shadow-sm flex flex-col items-center justify-center gap-3 text-slate-400 px-6 text-center">
    ...
    <>
      ...
      <div className="flex items-center justify-between mt-6">
        <button
          ...
          onClick={goPrev}
          disabled={index === 0}
          className="flex items-center gap-1 px-4 py-2 text-sm font-bold text-slate-500 rounded-xl border border-slate-200 bg-white disabled:opacity-40 disabled:cursor-not-allowed hover:border-rose-200 hover:text-rose-500 transition-colors"
        >
          <ChevronLeft size={16} />
